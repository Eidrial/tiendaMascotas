@extends('layouts.app')

<link rel="stylesheet" href="{{ asset('css/tienda.css') }}">

@section('content')

    <div class="admin-menu">
        <div class="admin-actions">
            <a href="{{ route('productos.create') }}" class="btn-add-product">Añadir nuevo producto</a>
            <a href="{{ route('admin.usuarios') }}" class="btn-add-product">Gestionar usuarios</a>
            <form action="{{ route('logout') }}" method="POST" class="logout-form">
                @csrf
                <input type="submit" value="Cerrar sesión" class="btn-simple logout">
            </form>
        </div>
    </div>


    <h2 class="titulo"><span class="titulo__color">MASCOTITAS</span> - ADMINISTRACIÓN</h2>

    @if (session('success'))
        <div class="alert-success">
            {{ session('success') }}
        </div>
    @endif

    <div class="grid-productos">
        @foreach ($productos as $producto)
            <div class="card-producto">
                <img src="{{ asset('storage/productos/' . $producto->imagen) }}" alt="{{ $producto->nombre }}">

                <div class="contenido">
                    <h3>{{ $producto->nombre }}</h3>
                    <p>{{ $producto->descripcion }}</p>
                    <p>
                        @if ($producto->stock > 0)
                            <span class="stock">Stock disponible:</span> {{ $producto->stock }}
                        @else
                            <span class="sin-stock">No hay stock disponible</span>
                        @endif
                    </p>
                    <p><b>{{ $producto->precio }} €</b></p>
                </div>

                <div style="display: flex; gap: 0.5rem; justify-content: center; margin-top: 1rem; flex-wrap: wrap;">
                    <!-- Ver detalles -->
                    <a href="{{ route('productos.show', $producto->id) }}" class="btn-comprar"
                        style="width: auto; padding: 8px 18px; font-size: 0.95rem; text-transform: none;">Ver</a>
                    <!-- Editar producto -->
                    <a href="{{ route('productos.edit', $producto->id) }}" class="btn-comprar"
                        style="width: auto; padding: 8px 18px; font-size: 0.95rem; text-transform: none; background: linear-gradient(90deg,#aee571 0%,#b2ff59 100%); color: #2d3e50;">Editar</a>
                    <!-- Eliminar producto -->
                    <a href="{{ route('productos.confirmar', $producto->id) }}" class="btn-comprar"
                        style="width: auto; padding: 8px 18px; font-size: 0.95rem; text-transform: none; background: linear-gradient(90deg,#ff8585 0%,#ffb347 100%); color: #2d3e50;">Eliminar</a>
                </div>
            </div>
        @endforeach
    </div>

    <div class="pagination">
        {{ $productos->links('pagination::simple-default') }}
    </div>
@endsection